...
${matchState.batsmen.map(batsman => 
  `${batsman.name}: ${batsman.runs} runs (${batsman.balls} balls, ${batsman.fours} fours, ${batsman.sixes} sixes)`
).join('\n')}
...
${matchState.dismissedBatsmen.map(batsman => 
  `${batsman.name}: ${batsman.runs} runs (${batsman.balls} balls, ${batsman.fours} fours, ${batsman.sixes} sixes) - ${batsman.dismissalType || 'Dismissed'}`
).join('\n')}` : ''}

BOWLING SUMMARY:
---------------
${matchState.bowlers.map(bowler => 
  `${bowler.name}: ${bowler.overs}.${matchState.balls} overs, ${bowler.runs} runs, ${bowler.wickets} wickets`
).join('\n')}

MATCH FORMAT: ${matchState.format.toUpperCase()}
Maximum Overs: ${matchState.maxOvers}
...
Generated by Tuskers CC Fan Scoring System
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `TuskersCC_Match_Report_${new Date().toISOString().split('T')[0]}.txt`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
